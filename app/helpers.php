...
function _format_param($item) {
	...
	if ($item["measurements"] != "n/a") {
		...
		if (sizeof($measurements) == 3) {
			$item["length"] = !empty($measurements[0]) ? floatval($measurements[0]) : "n/a";
			$item["width"] = !empty($measurements[1]) ? floatval($measurements[1]) : "n/a";
			$item["depth"] = !empty($measurements[2]) ? floatval($measurements[2]) : "n/a";
